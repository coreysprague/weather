...
class ForecastBackgroundRetriever {
	...
	func getImage(currentForecast: CurrentWeatherResponse, localTimeZone: TimeZone) -> String {
	...
	private func getBackgroundImage(timeOfDay: String, weatherCondition: WeatherCondition) -> String {
		...
		if let imageCategory = codeSpecificLookup[weatherCondition.conditionId] {
			return "\(imageCategory)_\(timeOfDay)"
		} else if let imageCategory = categoryLookup[weatherCondition.main] {
			return "\(imageCategory)_\(timeOfDay)"
		} else if weatherCondition.conditionId >= 700 && weatherCondition.conditionId < 800 {
